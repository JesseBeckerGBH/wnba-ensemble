-- ─────────────────────────────────────────────────────────────────────────────
-- NBA Betting DB — DuckDB schema (adapted from WNBA)
-- ─────────────────────────────────────────────────────────────────────────────

-- Teams (all 30 NBA franchises)
...
CREATE TABLE IF NOT EXISTS paper_trades (
    ...
    market        VARCHAR NOT NULL,  -- 'ML_HOME', 'ML_AWAY', 'OVER', 'UNDER'
    pick          VARCHAR NOT NULL,
    odds          REAL NOT NULL,
    model_prob    REAL NOT NULL,
    edge          REAL NOT NULL,
    wager         REAL NOT NULL,
    result        VARCHAR,           -- 'WIN', 'LOSS', 'PUSH', NULL (pending)
    pnl           REAL,
    bankroll_after REAL
);
...
CREATE OR REPLACE VIEW paper_performance AS
SELECT
    COUNT(*) AS total_trades,
    SUM(CASE WHEN result = 'WIN' THEN 1 ELSE 0 END) AS wins,
    SUM(CASE WHEN result = 'LOSS' THEN 1 ELSE 0 END) AS losses,
    ROUND(SUM(CASE WHEN result = 'WIN' THEN 1.0 ELSE 0.0 END) / NULLIF(COUNT(*), 0) * 100, 1) AS win_pct,
