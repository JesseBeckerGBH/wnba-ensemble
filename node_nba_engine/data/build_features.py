#!/usr/bin/env python3
"""
node_nba_engine/data/build_features.py

Compute ML features for every completed NBA game → game_features table.
Same architecture as WNBA engine, tuned for NBA pace/scale.

Usage:
    python data/build_features.py
    python data/build_features.py --season 2025
"""
...
def build_features(conn: duckdb.DuckDBPyConnection, season_filter: int | None = None):
    where = "WHERE g.home_score IS NOT NULL"
    if season_filter:
    ...
    rows_written = 0
    if season_filter:
        conn.execute("DELETE FROM game_features WHERE season_year = ?", [season_filter])
    else:
...
def main():
    p = argparse.ArgumentParser(description="Build NBA game features")
